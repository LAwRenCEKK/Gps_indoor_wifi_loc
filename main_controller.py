...
class MainController:
    ...
    def load_data(self):        
        ...
        # self.wifi_locations = np.load(self.import_dir+'wifi_locations.npy')[40:93]
        # self.wifi_values = np.zeros((len(self.wifi_locations),self.number_of_access_points))
        # print "this is wifi locations: " + str(self.wifi_locations.shape)
        # print self.wifi_locations
        # print "this is wifi rssi: " + str(self.wifi_rssi.shape)
        # print self.wifi_rssi[1]
        # print "this is wifi values: " + str(self.wifi_values.shape)
        # print self.wifi_values
        df = pd.read_csv('./source/dataset_gps.csv') 
        self.wifi_locations = df[['lat','lon']].to_numpy()
        self.wifi_rssi = df[['ap1','ap2','ap3','ap4']].to_numpy()
        self.wifi_values = np.zeros((len(self.wifi_locations),self.number_of_access_points))
   ...
   #Visualize wifi data       
    ...
    def preprocess_wifi_data(self,  wifi_locations, wifi_rssi, ap):               
        ...
        print "this is mean for rssi"
        print wifi_rssi_mean
        print wifi_rssi_mean.shape
    ...
    def visualize_gaussian(self, ap_index, gp):
        ...
        X_range = np.arange(30,65,1)
        Y_range = np.arange(10,80,1)
